FILE_VERSION = "1.4.61"; -- this is the only global which is allowed to be outside of RegisterGlobals()
	...
	function g_localActor.Client:OnHit(hit, remote)
		...
		if(hit.target and hit.shooter and hit.weapon and hit.weapon.class~="Fists")then
			...
			if (hits > 0 and splat and ((splat.dist or 0)<1.5)) then
				...
				local a = Particle.CreateMatDecal(splat.pos, splat.normal, math.random(0,8,1.3), 300, hit.target.bloodSplatWall[math.random(#hit.target.bloodSplatWall)], math.random()*360, splat.dir, splat.entity.id, splat.renderNode);
